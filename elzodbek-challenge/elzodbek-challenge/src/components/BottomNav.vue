...
const navItems = [
  ...
  { path: '/alarm',     icon: '⏰',  label: 'nav.alarm' },
...
.nav-item { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; padding: 4px 2px; border-radius: var(--radius-sm); text-decoration: none; color: var(--text-dim); transition: all 0.2s; border: none; background: none; cursor: pointer; font-family: var(--font-body); min-width: 0; }
.nav-item:hover, .nav-item.router-link-active { color: var(--accent-light); }
[data-theme="light"] .nav-item.router-link-active { color: var(--accent); }
.nav-item.router-link-active .nav-icon { transform: scale(1.15); }
